...
The smiley plot was generated from a double-stranded, non-UDG treated library and displays an increase in the G→A and C→T misincorporation frequencies at the first base of the 3' termini. The library was preprocessed with AdapterRemoval v2.1.7, which performs adapter trimming and paired-end merging based on sequence overlap. A known limitation of this approach is that asymmetric read pairs (e.g., reads with leading Ns or differing insert sizes) can result in incomplete adapter removal or loss of authentic fragment termini. In this instance, a single 3' terminal adapter base remained, leading to an artificial inflation of the damage frequency.

In case you get a similar plot and you still have your raw FASTQ files (and you probably do), you can easily reprocess them with a cleaner approach — e.g. Cutadapt + Flash (or another preferred tool for trimming/merging).
